 ...
 **/
@Slf4j
public class Second2DataListener extends AnalysisEventListener<Second2Data> {
     ...
     */
    @Override
    public void invoke(Second2Data data, AnalysisContext context) {
        if (DB.containsKey(data.getText0().getTime())) {
            Map<String, Object> value = DB.get(data.getText0().getTime());
            if (!value.containsKey("second2")) {
                value.put("second2", data);
            }
        } else {
